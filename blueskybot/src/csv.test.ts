...
describe('parseCsvRow', () => {
    ...
    it('handles quoted field containing commas', () => {
        assert.deepEqual(
            ...
            [
                '01-02',
                'Granada, the last Moorish stronghold, surrenders',
                '1492',
            ],
        );
    });
    ...
    it('handles quoted field at the end of the row', () => {
        assert.deepEqual(parseCsvRow('01-01,"Event, with commas"'), [
            '01-01',
            'Event, with commas',
        ]);
    });
    ...
    it('handles a mix of quoted and unquoted fields', () => {
        assert.deepEqual(parseCsvRow('date,"event, text",1999'), [
            'date',
            'event, text',
            '1999',
        ]);
    ...
    it('handles row from the custom calendar with commas in quotes', () => {
        assert.deepEqual(
            ...
            [
                'January 2',
                '1492: Granada, the last Moorish stronghold in España, surrenders',
            ],
        );
    });
    ...
    it('handles parenthetical text in quoted field', () => {
        assert.deepEqual(
            ...
            [
                'January 13',
                '1910: First public radio broadcast (Met Opera, NYC)',
            ],
        );
    });
});
...
describe('fieldsToEvent', () => {
    ...
    it('rejoins 4+ fields with empty year', () => {
        assert.deepEqual(fieldsToEvent(['01-01', 'A', ' B', '']), {
            date: '01-01',
            event: 'A, B',
            year: undefined,
        });
...
describe('loadEvents', () => {
    it('loads events.csv and returns all 366 entries', () => {
        const events = loadEvents(
            new URL('../data/events.csv', import.meta.url).pathname,
        );
...
describe('getEventsForToday — leap year and date boundaries', () => {
    ...
    it('returns no Feb 29 match on any day of a non-leap year', () => {
        ...
        for (let day = 1; day <= 28; day++) {
            ...
            assert.ok(
                result.every((e) => e.date !== '02-29'),
                `Unexpected Feb 29 match on Feb ${day}`,
            );
        }
    });
